...
function Working_Yoga(){
    ...
    const fetchAndSetData = async () => {
      try {
            const exerciseID = '15';
            const difficulty = 2;
            const exerciseResponse = await axios.get(`https://wellbeing.htcangelfund.com/api/exercise/exercises/${exerciseID}/`);
                            ...
                            <Grid container item direction="column" alignItems="center" justifyContent="center">
                                <Grid container item direction="row" alignItems="center" justifyContent="flex-start"
                                    ...
                                    <Typography variant="h5"
                                                sx={{fontWeight: 'bold', lineHeight: 1.5, m: 2, fontFamily: 'MSYH'}}>
                                        {showIndex <= imgs.length - 1 ? imgs[showIndex].label : imgs[0].label}
                                    </Typography>
                                    {showIndex < imgs.length - 1
                                        ? <Typography variant="h6" sx={{fontWeight: 'bold', lineHeight: 1.5, ml: 45, fontFamily: 'MSYH'}}> >> Next: {imgs[showIndex + 1].label} </Typography>
                                        : <Typography variant="h6" sx={{fontWeight: 'bold', lineHeight: 1.5, ml: 45, fontFamily: 'MSYH'}}> </Typography>
                                    }
                                </Grid>
